...
class EnsembleModel(nn.Module):
    ...
    @staticmethod
    def loss(mean: torch.Tensor, labels: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Returns the loss
...
class EnsembleDynamicsModel:
    ...
    def predict(self, inputs: np.ndarray, batch_size: int = 1024, n_samples: int = 10) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        :param n_samples: How many samples to use for MC-dropout
