...
def pendulum() -> None:
    ...
    kmeans_time_array = np.empty_like(nn_time_array)
    ...
    for n in range(N_EVAL):
        ...
        kmeans_minibatch = MiniBatchKMeans(n_clusters=n_clusters, random_state=SEED, batch_size=2048, reassignment_ratio=0)
        kmeans_minibatch_fast = MiniBatchKMeans(n_clusters=n_clusters, random_state=SEED, batch_size=2048, reassignment_ratio=0)
        kmeans = sklearnKmeans(n_clusters=n_clusters, random_state=SEED, max_iter=100)
        clusters_minibatch_kmeans = [StandardScaler() for _ in range(n_clusters)]
        clusters_kmeans = [[] for _ in range(n_clusters)]
        clusters_kmeans_gpu = [[] for _ in range(n_clusters)]
        ...
        for k, eval_timesteps in enumerate(eval_at_timesteps):
            ...
            kmeans = kmeans_minibatch.partial_fit(z_space_norm)

            # max startup steps, else max max_timesteps
            labels = kmeans.labels_
            z_space = np.concatenate((z_space, r, o_2), axis=-1)
            for m in range(n_clusters):
                indices = np.where(labels == m)[0]
                if indices.size > 0:
                    clusters_minibatch_kmeans[m].partial_fit(z_space[indices])   
            # for m in range(len(labels)):
            #     clusters_minibatch_kmeans[labels[m]] = clusters_minibatch_kmeans[labels[m]].partial_fit(z_space[m].reshape(1, -1))
            ...
            kmeans = kmeans_minibatch_fast.partial_fit(z_space_norm)  
            ...
            # Construct Z-space
            z_space = np.concatenate((obs[:eval_timesteps], actions[:eval_timesteps]), axis=-1)
            scaler_kmeans.partial_fit(z_space)
            z_space_norm = scaler_kmeans.transform(z_space)
            labels = kmeans.fit_predict(z_space_norm)
            ...
            for m in range(n_clusters):
                ...
                clusters_kmeans[n] = buffer_idx

            end = time.time()
            kmeans_time_array[k, n] = end - start
            ...
            for m in range(n_clusters):
                ...
                clusters_kmeans_gpu[n] = buffer_idx
    ...
    kmeans_time_median = np.median(kmeans_time_array, axis=-1)
    ...
    data = {
        ...
        "kmeans_time_array": kmeans_time_array,
...
def hand():
    ...
    for n in range(N_EVAL):
        ...
        minibatch_kmeans_fast_size = state_dim + goal_dim + action_dim + goal_dim + state_dim + goal_dim
        minibatch_kmeans_fast_sum = np.zeros(shape=(n_clusters, minibatch_kmeans_fast_size))
        minibatch_kmeans_fast_sum_sq = np.zeros(shape=(n_clusters, minibatch_kmeans_fast_size))
        minibatch_kmeans_fast_count = np.zeros(shape=(n_clusters, 1))
        minibatch_kmeans_fast_mean = np.zeros(shape=(n_clusters, minibatch_kmeans_fast_size))
