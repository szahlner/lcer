...
def _check_returned_values(env: gym.Env, observation_space: spaces.Space, action_space: spaces.Space) -> None:
    ...
    assert isinstance(reward, (bool, float, np.floating, int)), "The reward returned by `step()` must be a float"
    assert isinstance(done, (bool, float, np.floating)), "The `done` signal must be a boolean"
