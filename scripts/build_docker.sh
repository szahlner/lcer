...
GPU_PARENT=nvidia/cuda:11.3.0-runtime-ubuntu20.04
...
echo "docker build --build-arg PARENT_IMAGE=${PARENT} --build-arg PYTORCH_INSTALL=${PYTORCH_INSTALL} -t ${TAG}:${VERSION} . -f ./Docker/Dockerfile"
docker build --build-arg PARENT_IMAGE=${PARENT} --build-arg PYTORCH_INSTALL=${PYTORCH_INSTALL} -t ${TAG}:${VERSION} . -f ./Docker/Dockerfile
